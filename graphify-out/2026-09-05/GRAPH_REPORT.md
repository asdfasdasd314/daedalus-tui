...
## Corpus Check
- 75 files · ~54,079 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1153 nodes · 3027 edges · 66 communities (59 shown, 7 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 319 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `59ee4c33`
...
### Community 4 - "app.py"
Cohesion: 0.15
Nodes (7): RowSelected, Path, Focus a task from the cross-project update inbox., Memory override for the project, else the parameter-file default., Update the project's coordinator so later submits use ``branch``., Refresh Branch Select options for the active project and sync coordinator., Switch the project context and focus a row selected in the inbox.
...
### Community 11 - "Key"
Cohesion: 0.10
Nodes (15): GitWorktreeTests, GitWorktreeError, GitWorktreeManager, list_local_branches(), push_branch(), RuntimeError, Local Git worktree lifecycle used by the standalone orchestrator., Install project resources and link declared shared read-only paths. (+7 more)
...
Cohesion: 0.14
Nodes (6): Changed, DaedalusTuiApp, Remember the selected topic as the default for the active project., Prefer mounted selector values so clarification refreshes keep choices., Replace question controls after Textual has completed child removal., Return a TextArea selection or the active screen selection.
...
Cohesion: 0.08
Nodes (11): Pressed, Submitted, CreateTopicScreen, PlanClarificationScreen, ProjectInitializerScreen, Collect a clarification about one plan question., Collect a project slug and create a Daedalus-compatible directory., Collect the context needed to initialize and populate a topic file. (+3 more)
...
Cohesion: 0.23
Nodes (3): Key, Clear the selected task and unlock a fresh prompt editor., Add Vim-like navigation without changing TextArea insert behavior.
...
Cohesion: 0.17
Nodes (4): ComposeResult, Select, KeyboardShortcutsScreen, Modal reference for the app and prompt editor keyboard shortcuts.
...
Cohesion: 0.19
Nodes (9): Enable Push only when an origin remote and operating branch are available., Push the Branch Select value for the active project to origin., configure_debug_logging(), install_fault_handler(), _install_thread_exception_logging(), Path, Persistent diagnostics for failures that occur after the Textual screen closes., Write detailed runtime diagnostics to a rotating local log file. (+1 more)
...
Cohesion: 0.11
Nodes (15): TaskEventCallback, _nonnegative_int(), _phase_for_status(), Path, Send selected plan answers back to the planning agent for confirmation., Ask a side-channel clarification about one plan question without plan follow-up., Run another planning pass while keeping the task in questioning., Promote a reviewed plan into the normal coding and verification route. (+7 more)

### Community 50 - "._render_line_strip"
Cohesion: 0.15
Nodes (8): FakeOrchestrator, OrchestrationResult, IntegrationCoordinator, Recover review questions from the last persisted plan response., Run ready integration operations one at a time in ready order., Create a new coding task from a confirmed plan review., Remove the clean, read-only planning worktree after coding is queued., TaskRecord

### Community 51 - "CreateTopicScreen"
Cohesion: 0.20
Nodes (13): Mount, ConfigTests, PlanAnswerSelect, Textual interface for concurrent local agent tasks., Initialize dynamic plan selectors after their nested children mount., load_coding_statistics_settings(), load_orchestration_settings(), load_tui_settings() (+5 more)
...
Cohesion: 0.29
Nodes (4): Refresh topics and restore the active project's remembered default., Return the selected topic slug, or None for the blank option., Enable topic viewing only when the selector has a real topic., Open the selected project topic in a read-only modal.

### Community 64 - "PlanClarification"
Cohesion: 0.40
Nodes (3): PlanClarification, A side-channel clarification about one plan question (not plan follow-up)., Run an ask-mode clarification that does not mutate plan conversation state.
...
- **Why does `DaedalusTuiApp` connect `._shutdown_coordinators` to `DaedalusVimTextArea`, `AgentRunner`, `DaedalusTuiApp`, `app.py`, `GitWorktreeManager`, `Key`, `TaskRecord`, `._set_status`, `update_repository`, `CodingStatisticsScreen`, `calculate_token_usage`, `.on_button_pressed`, `agent_runner.py`, `._set_status`, `._rebuild_plan_questions`, `task_coordinator.py`, `._start_new_task`, `._render_line_strip`, `CreateTopicScreen`, `._render_selected_task`, `CodingStatisticsScreen`, `.action_view_topic`, `PlanClarification`, `._render_selected_task`?**
  _High betweenness centrality (0.187) - this node is a cross-community bridge._
- **Why does `TaskMemoryStore` connect `calculate_token_usage` to `.on_button_pressed`, `agent_runner.py`, `._rebuild_plan_questions`, `task_coordinator.py`, `PlanQuestion`, `._render_line_strip`, `CreateTopicScreen`, `._shutdown_coordinators`, `CodingStatisticsScreen`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `TaskCoordinator` connect `task_coordinator.py` to `PlanClarification`, `.on_button_pressed`, `AgentRunner`, `agent_runner.py`, `app.py`, `KeyboardShortcutsScreen`, `._rebuild_plan_questions`, `Key`, `.on_data_table_row_selected`, `PlanQuestion`, `._render_line_strip`, `CreateTopicScreen`, `._shutdown_coordinators`, `CodingStatisticsScreen`, `calculate_token_usage`?**

...
## Corpus Check
- 75 files · ~52,687 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1134 nodes · 2962 edges · 59 communities (52 shown, 7 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 314 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c068ad5d`
...
### Community 4 - "app.py"
Cohesion: 0.11
Nodes (10): Path, Queue a coding task that writes and expands the requested topic file., Memory override for the project, else the parameter-file default., Update the project's coordinator so later submits use ``branch``., Refresh topics and restore the active project's remembered default., Render actionable history and tasks created during this session., Keep failures, active work, and all tasks submitted in this launch., Switch the project context and focus a row selected in the inbox. (+2 more)
...
Cohesion: 0.15
Nodes (12): PlanClarification, A side-channel clarification about one plan question (not plan follow-up)., Send selected plan answers back to the planning agent for confirmation., Ask a side-channel clarification about one plan question without plan follow-up., Create a new coding task from a confirmed plan review., Run another planning pass while keeping the task in questioning., Promote a reviewed plan into the normal coding and verification route., Retry a failed agent request after connectivity or service recovery. (+4 more)

### Community 21 - "._shutdown_coordinators"
Cohesion: 0.12
Nodes (8): Changed, DaedalusTuiApp, Promote only events that need the user's attention in the inbox., Remember the selected topic as the default for the active project., Prefer mounted selector values so clarification refreshes keep choices., Replace question controls after Textual has completed child removal., Show either the full-size transcript or the full-size diagnostics log., Use Textual's OSC 52 path and a native clipboard fallback.

### Community 22 - "._set_status"
Cohesion: 0.33
Nodes (4): paste_from_system_clipboard(), Read clipboard text using the native command available on the host., Paste Vim's register, falling back to the system clipboard., Paste before the cursor, including from the system clipboard.
...
Cohesion: 0.25
Nodes (3): Submitted, CreateTopicScreen, Collect the context needed to initialize and populate a topic file.
...
Cohesion: 0.14
Nodes (3): TaskCoordinatorTests, Submit independent prompts while sharing a serialized integration gate., TaskCoordinator
...
Cohesion: 0.10
Nodes (27): EventCallback, IntegrationGate, OrchestratorTests, VerificationTests, AgentControl, AgentResult, Cooperative stop signals shared by a task and its active subprocess., WorktreeContext (+19 more)
...
Cohesion: 0.11
Nodes (8): Pressed, PlanClarificationScreen, ProjectInitializerScreen, Collect a clarification about one plan question., Collect a project slug and create a Daedalus-compatible directory., Display a topic markdown file without allowing edits., TopicViewerScreen, CodingStatisticsSettings

### Community 44 - "task_coordinator.py"
Cohesion: 0.21
Nodes (10): TaskEventCallback, _nonnegative_int(), _phase_for_status(), Path, Concurrent task state and serialized local integration., Rehydrate this project's persisted tasks after a TUI restart., _snapshot_path(), _snapshot_sequence() (+2 more)
...
Cohesion: 0.22
Nodes (4): Refresh Branch Select options for the active project and sync coordinator., Enable Push only when an origin remote and operating branch are available., Push the Branch Select value for the active project to origin., Cover terminal and event-loop exits that bypass Textual unmount.
...
Cohesion: 0.15
Nodes (15): ConfigTests, KeyboardShortcutsScreen, Textual interface for concurrent local agent tasks., Modal reference for the app and prompt editor keyboard shortcuts., copy_to_system_clipboard(), System clipboard helpers for terminals without OSC 52 support., load_coding_statistics_settings(), load_orchestration_settings() (+7 more)
...
- **Why does `DaedalusTuiApp` connect `._shutdown_coordinators` to `DaedalusVimTextArea`, `AgentRunner`, `DaedalusTuiApp`, `app.py`, `GitWorktreeManager`, `Key`, `TaskRecord`, `update_repository`, `CodingStatisticsScreen`, `calculate_token_usage`, `.on_button_pressed`, `._set_status`, `._rebuild_plan_questions`, `._start_new_task`, `.on_data_table_row_selected`, `PlanQuestion`, `CreateTopicScreen`, `.action_view_topic`, `._render_selected_task`?**
  _High betweenness centrality (0.216) - this node is a cross-community bridge._
- **Why does `GitWorktreeError` connect `Key` to `PlanQuestion`, `DaedalusVimTextArea`, `.on_button_pressed`, `agent_runner.py`, `._set_status`, `update_repository`, `KeyboardShortcutsScreen`, `._rebuild_plan_questions`, `task_coordinator.py`, `.emit`, `CreateTopicScreen`, `TaskRecord`, `._shutdown_coordinators`, `CodingStatisticsScreen`?**
  _High betweenness centrality (0.082) - this node is a cross-community bridge._
- **Why does `TaskMemoryStore` connect `calculate_token_usage` to `PlanQuestion`, `.on_button_pressed`, `agent_runner.py`, `._set_status`, `._rebuild_plan_questions`, `task_coordinator.py`, `.emit`, `CreateTopicScreen`, `TaskRecord`, `._shutdown_coordinators`?**

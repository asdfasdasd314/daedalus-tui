...
## Corpus Check
- 75 files · ~53,185 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1142 nodes · 2989 edges · 58 communities (50 shown, 8 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 314 edges (avg confidence: 0.55)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9eff3eba`
...
1. `DaedalusTuiApp` - 132 edges
2. `TuiAppTests` - 85 edges
...
Cohesion: 0.13
Nodes (3): Agent plan text must not be parsed as Textual/Rich markup., Follow-up questions must not crash when old option ids linger on widgets., TuiAppTests
...
### Community 4 - "app.py"
Cohesion: 0.14
Nodes (7): Path, Queue a coding task that writes and expands the requested topic file., Memory override for the project, else the parameter-file default., Update the project's coordinator so later submits use ``branch``., Render actionable history and tasks created during this session., Keep failures, active work, and all tasks submitted in this launch., Switch the project context and focus a row selected in the inbox.
...
Cohesion: 0.36
Nodes (3): CodingStatisticsScreen, _format_count(), Show token or task usage history and derived coding statistics.
...
Cohesion: 0.12
Nodes (6): ComposeResult, Submitted, CreateTopicScreen, ProjectInitializerScreen, Collect a project slug and create a Daedalus-compatible directory., Collect the context needed to initialize and populate a topic file.
...
Cohesion: 0.13
Nodes (7): Pressed, PlanClarificationScreen, Collect a clarification about one plan question., Display a topic markdown file without allowing edits., TopicViewerScreen, PlanClarification, A side-channel clarification about one plan question (not plan follow-up).
...
Cohesion: 0.05
Nodes (52): TaskEventCallback, PlanTests, FakeOrchestrator, TaskCoordinatorTests, Resolve a Select value that is legal for the current question options., OrchestrationResult, OrchestrationSettings, build_implementation_prompt() (+44 more)
...
- **Why does `DaedalusTuiApp` connect `._shutdown_coordinators` to `DaedalusVimTextArea`, `AgentRunner`, `DaedalusTuiApp`, `app.py`, `GitWorktreeManager`, `Key`, `TaskRecord`, `._set_status`, `update_repository`, `CodingStatisticsScreen`, `calculate_token_usage`, `._set_status`, `._rebuild_plan_questions`, `task_coordinator.py`, `._start_new_task`, `.on_data_table_row_selected`, `PlanQuestion`, `CreateTopicScreen`, `.action_view_topic`, `._render_selected_task`?**
  _High betweenness centrality (0.223) - this node is a cross-community bridge._
- **Why does `TaskMemoryStore` connect `calculate_token_usage` to `PlanQuestion`, `.on_button_pressed`, `agent_runner.py`, `._rebuild_plan_questions`, `task_coordinator.py`, `PlanQuestion`, `CreateTopicScreen`, `._shutdown_coordinators`, `._set_status`?**
  _High betweenness centrality (0.081) - this node is a cross-community bridge._
- **Why does `GitWorktreeError` connect `Key` to `PlanQuestion`, `DaedalusVimTextArea`, `.on_button_pressed`, `agent_runner.py`, `update_repository`, `KeyboardShortcutsScreen`, `._rebuild_plan_questions`, `task_coordinator.py`, `PlanQuestion`, `CreateTopicScreen`, `._shutdown_coordinators`, `._set_status`, `CodingStatisticsScreen`?**

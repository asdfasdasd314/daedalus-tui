...
## Summary
Standalone tooling that statically analyzes a target Python project with pyan3 and writes a self-contained, top-down HTML call tree for import and feature-boundary workflows. The runner can analyze the current directory or any named checkout under a configurable projects root, with optional per-project root and analysis overrides. Related call-graph symbols also receive exploratory TF-IDF cosine similarity scores so reviewers can judge whether high similarity tracks cohesive feature neighborhoods. A first-pass AST variable-lineage analysis adds scoped variable/attribute usage stats as additional import evidence for later resource and key-point detection.

## Key Points
- **Parameter-driven target selection**: `parameter_files/daedalus-tui-call-graph-visualization.toml` owns `analysis_project` and `projects_root`. `current` means the working directory; any other project name automatically resolves to `<projects_root>/<name>`. Optional `[projects."name"]` tables support a nonstandard root or project-specific overrides without code changes.
- **Flexible project defaults and overrides**: `project_defaults` uses recursive Python discovery with common generated/dependency/test exclusions, so a non-Daedalus checkout can be analyzed before it has a `parameter_files` directory. An analyzed project's own parameter file takes final precedence when present.
...
- `scripts/render_call_graph_tree.py`: Runnable entrypoint; resolves the parameter-selected target and merges generic/project/target-local parameters.
- `parameter_files/daedalus-tui-call-graph-visualization.toml`: Target selection, generic import defaults, current-TUI defaults, and optional project overrides.
- `tests/test_call_graph_tree.py`: Unit tests against a small fixture package.
- `tests/test_call_graph_runner.py`: Regression tests for generic project defaults, target-local overrides, and root selection.
...
- 2026-09-10: Moved analysis selection into the parameter file and added automatic `<projects_root>/<name>` resolution plus optional per-project overrides, so arbitrary sibling checkouts no longer require runner code changes.
